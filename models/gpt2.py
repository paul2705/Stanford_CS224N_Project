...
class GPT2Model(GPTPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### YOUR CODE HERE
    # word embeddings [bs, seq_len, hidden]
    inputs_embeds = self.word_embedding(input_ids)

    # positional embeddings [1, seq_len, hidden]
    pos_ids = self.position_ids[:, :seq_length]
    pos_embeds = None

    ### DONE: Use pos_ids to get position embedding from self.pos_embedding into pos_embeds.
  ...
  def hidden_state_to_token(self, hidden_state):
    ...
    ### YOUR CODE HERE
    """
    hidden_state: [bs, hidden] or [bs, seq_len, hidden]
    returns logits: [bs, vocab] or [bs, seq_len, vocab]
    """
